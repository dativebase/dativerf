...
(doseq [[event path]
        {::user-clicked-forms-labels-button
         ...
         ::user-clicked-new-search-forms-button
...
(defn- invalidate-search-cache [db]
       (assoc db
              :new-form nil
              :new-form-general-validation-error-message nil
              :new-form-field-specific-validation-error-messages {}))
...
(re-frame/reg-event-fx
  ::initiated-form-search
  (fn-traced [{:keys [db]} [event]]
             {:db (transition-new-form-fsm db event)
              :http-xhrio
              (assoc post-request
                     :uri (old/formsearches-new (old-model/old db))
                     :params (utils/->snake-case-recursive
                               (form-model/new-form db))
                     :on-success []
                     :on-failure [])}))
...
(re-frame/reg-event-fx
  ::user-clicked-search-forms-button
  (fn-traced [{:keys [db]} _]
             {:dispatch [::initiated-form-search]}))

(re-frame/reg-event-db
  ::user-clicked-clear-search-forms-interface
  (fn-traced [db _] (merge db db/default-search-state)))

(re-frame/reg-event-db
  ::user-clicked-help-search-forms
  ;; TODO
  (fn-traced [_db _]))
